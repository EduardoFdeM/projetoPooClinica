...
<html lang="pt-BR">
  <head>
    ...
    <title>Gerenciar Médicos - Clínica Médica</title>
    <link
      rel="stylesheet"
    ...
    <div class="container">
      ...
      <c:if test="${param.error != null}">
        <div class="alert alert-danger">
          <c:choose>
            <c:when test="${param.error == 'medico_nao_encontrado'}">
              Médico não encontrado.
            </c:when>
            <c:when test="${param.error == 'falha_criar'}">
              Erro ao cadastrar o médico. Tente novamente.
            </c:when>
            <c:when test="${param.error == 'falha_atualizar'}">
              Erro ao atualizar o médico. Tente novamente.
            </c:when>
            <c:when test="${param.error == 'falha_deletar'}">
              Erro ao excluir o médico. Tente novamente.
            </c:when>
            <c:otherwise>
      ...
      <div class="actions-container">
        <a
          href="${pageContext.request.contextPath}/medicos?action=new"
          ...
          <table>
            ...
            <tbody>
              <c:forEach var="medico" items="${medicos}">
                <tr>
                  <td data-label="ID:">${medico.id}</td>
                  <td data-label="Nome:">${medico.nome}</td>
                  <td data-label="Email:">${medico.email}</td>
                  <td data-label="CPF:">${medico.cpf}</td>
                  <td data-label="Celular:">
                    ${medico.celular != null ? medico.celular : '-'}
                  </td>
                  <td data-label="Ações:" class="actions">
                    <a
                      href="${pageContext.request.contextPath}/medicos?action=edit&id=${medico.id}"
      ...
      function confirmarExclusao(element) {
        ...
        if (
          ...
          window.location.href =
            '${pageContext.request.contextPath}/medicos?action=delete&id=' + id
        }
      }
    </script>
  </body>
